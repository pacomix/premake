...
--
    ...
    for _, fname in ipairs(prj.resources) do
      --printf("\t\t" .. fname)
			local node = premake.tree.add(tr, fname)
      --printf("Added node name: " .. node.name)
...
--
  ...
  function premake.getfileconfig(prj, fname)
    if not fname then return nil end
    return prj.__fileconfigs[fname]
  end
...
--
    ...
    if pltname == "Native" or not table.contains(prj.platforms or {}, pltname) then
			pltname = nil
		end
...
--
  ...
  function premake.getreferences(prj)
		-- make sure I've got the project and not root config
		prj = prj.project or prj
		
		local results = { }
		for _, cfg in pairs(prj.__configs) do
			for _, link in ipairs(cfg.linksReferences) do
				local dep = premake.findproject(link)
				if dep and not table.contains(results, dep) then
					table.insert(results, dep)
				end
  ...
  function premake.getassembliesreferences(prj)
		-- make sure I've got the project and not root config
		prj = prj.project or prj
		...
		for _, cfg in pairs(prj.__configs) do
			for _, link in ipairs(cfg.linksReferences) do
				local dep = premake.findproject(link)
				if not dep and not table.contains(results, link) then
...
--	
 	...
 	function premake.getlinks(cfg, kind, part)
      ...
      -- If target doesn't exist we can not link. By: Captain Obvious
      ...
      --printf("Looking for link lib/dependency: '%s' for config: '%s|%s'", link, cfgname, cfg.platform)
			...
			elseif not prj and (kind == "system" or kind == "all") then
              ...
              if item ~= "" and not (string.startswith(item, "-l") or (string.startswith(item, '"') and string.endswith(item, '"')) or string.endswith(item, ".lib") or string.endswith(item, '.lib"') or string.endswith(item, ".a") or string.endswith(item, ('.a"') or string.endswith(item, ".ppu.obj") or string.endswith(item, ('.ppu.obj"')))) then
                item = item .. ".lib"
              end
						elseif premake.isdotnetproject(cfg) then
              if item ~= ""  and not (string.startswith(item, "-l") or string.endswith(item, ".dll") or string.endswith(item, '.dll"')) then
                item = item .. ".dll"
              end
						end
					end
...
--
	...
	function premake.gettarget(cfg, direction, pathstyle, namestyle, system)
		...
		local name    = cfg[field.."name"] or cfg.targetname or cfg.project.nicename or cfg.project.name
    ...
    elseif namestyle == "Android" then
		...
		result.fullpath   = result.directory .. result.name--path.join(result.directory, result.name)
    ...
    --[[
    printf("\t\t\t\t\t: '%s'", result.basename)
    printf("\t\t\t\t\t: '%s'", result.name)
    printf("\t\t\t\t\t: '%s'", result.directory)
    printf("\t\t\t\t\t: '%s'", result.prefix)
    printf("\t\t\t\t\t: '%s'", result.suffix)
    printf("\t\t\t\t\t: '%s'", result.fullpath)
    printf("\t\t\t\t\t: '%s'", result.bundlepath)
    --]]
