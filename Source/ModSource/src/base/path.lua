...
--
    ...
    if string.findlast(p, '$', true) then return p end
...
--
    ...
    --if string.findlast(dst, '$', true) then
			return dst
		end
		...
		src = src .. "/"
		dst = dst .. "/"
    --printf("Absolute parent: " .. src .. " Son: " .. dst)
		...
		while (true) do
          ...
          --printf("/ found: " .. src:sub(1,tst) .. " == " .. dst:sub(1,tst))
					idx = tst
				else
          --printf("Not equal src<->dst: " .. src:sub(1,tst) .. "<->" .. dst:sub(1,tst))
					break
				end
			else
        --printf("No directory division / found")
				break
			end
		end
		...
		if idx < first then
      --printf("Returning absolute!!!!" .. dst:sub(1, -2))
			return dst:sub(1, -2)
		end
