...
--
	...
	function premake.config.isdebugbuild(cfg)
		...
		-- TODO - Add a way to identify if the build is a debug or release. Use a Flag for that instead.
...
--
	...
	local function collapse(obj, basis)
		...
		-- find the solution, which contains the configuration and platform lists <------ that is completely WRONG! A Project can have a different set of build configurations. Then from the solution properties the build configuration can be mapped to a valid one in the project!
    
    local sln = nil
    
    if _ACTION ~= "xcode4" or _ACTION ~= "xcode3" then
      sln = obj --.solution or obj
    else
      sln = obj.solution or obj
    end
...
--
	...
	local function builduniquedirs()
		...
		for sln in premake.solution.each() do
          ...
          -- Temp. Only used to test that the rests of the platforms and new ps3 configs are ok.
          if cfg.platform == "PS3" then
            --dirs[2] = path.join(dirs[1], "x32")
            --dirs[3] = path.join(dirs[2], cfg.name .. " PS3")
		...
		for sln in premake.solution.each() do
          ...
          --printf("Getting relative. SRC:")
          --printf("\t"..cfg.location)
          --printf("\t"..dir)
					cfg.objectsdir = path.getrelative(cfg.location, dir)
          --printf("Result: " .. cfg.objectsdir)
				end
			end
		end		
...
--
      ...
      printf("\t\tSolution:\t'%s'", sln.name)
        ...
        printf("\t\t\t'%s'", prj.name)
				for _, cfg in pairs(prj.__configs) do
					...
					-- build the targets
          --printf("\t\t\t\tTarget res: '%s'", iif(cfg.name and cfg.name ~= "", cfg.name, "UNKNOWN"))
	...
	function premake.printConfigurations(prj, text)
    printf(text)
    printf("Configs:")
    for _, i in ipairs(prj.configurations) do
      printf("\t" .. i)
    end
    
    printf("Platfs:")
    for _, i in ipairs(prj.platforms) do
      printf("\t" .. i)
    end
  end
...
--
    ...
    printf("\tConverting absolute paths to relative...")
		for sln in premake.solution.each() do
      printf("\t\tSolution:\t'%s'", sln.name)
			for _, prj in ipairs(sln.projects) do
        printf("\t\t\t'%s'", prj.name)
    ...
    -- NOTE: This also collapses project and solution objects.
    printf("\tCollapsing configuration blocks...")
		for sln in premake.solution.each() do
      printf("\t\tSolution:\t'%s'", sln.name)
        ...
        --printf("\t\t\t\tPostprocessing...", prj.name)
				for _, cfg in pairs(prj.__configs) do
					postprocess(prj, cfg)
				end
        io.write("\n")
    ...
    printf("\tBuilding unique directories...")
		builduniquedirs()
		
		-- walk it again and build the targets and unique directories
    printf("\tBuilding targets & unique directories...")
		buildtargets(cfg)
