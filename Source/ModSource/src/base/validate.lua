		...
		for sln in premake.solution.each() do
			...
			for prj in premake.solution.eachproject(sln) do
				...
				for cfg in premake.eachconfig(prj) do
					...
					if (not cfg.kind) then -- TODO - This check should be done for configuration and platform. This arises wrong error in this way. Commented out.
						--return nil, "project '" ..prj.name .. "' needs a kind in configuration '" .. cfg.name .. "'"
            --printf ("'WARNING: Project '%s' has no kind defined in build configuration:\t'%s'", prj.name, cfg.name)
            --continue
          else   
            --printf ("'Taking build from project '%s':\t'%s|%s'", prj.name, cfg.name, cfg.platform)
            -- and the action must support it
