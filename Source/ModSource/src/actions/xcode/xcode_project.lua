...
--
    ...
    printf("Building configuration list with their IDs...")
		tr.configs = {}
		for _, cfgname in ipairs(prj.solution.configurations) do
			for _, platform in ipairs(prj.solution.xcode.platforms) do
      
        -- Check if the config is marked to be built in the project
      
				local cfg = premake.getconfig(prj, cfgname, platform)
				cfg.xcode = {}
				cfg.xcode.targetid = xcode.newid(prj, prj.xcode.projectnode, cfgname)
				cfg.xcode.projectid = xcode.newid(prj, tr, cfgname)
    ...
    printf("Converting localized resources...")
		tree.traverse(tr, {
    ...
    local bEinMal = true
      ...
      if bEinMal == true then
        printf("\t\tAdding project dependencies:")
        bEinMal = false
      end
      printf("\t\t\t- " .. dep.name)
			...
			xcnode.path = xcpath
			xcnode.project = dep
			xcnode.productgroupid = xcode.newid(prj, xcnode, "prodgrp")
			xcnode.productproxyid = xcode.newid(prj, xcnode, "prodprox")
			xcnode.targetproxyid  = xcode.newid(prj, xcnode, "targprox")
			xcnode.targetdependid = xcode.newid(prj, xcnode, "targdep")
		...
		-- Final setup
    printf("Generating IDs for everything...")
		tree.traverse(tr, {
			onnode = function(node)
				-- assign IDs to every node in the tree
				node.id = xcode.newid(prj, node)
        
				-- assign build IDs to buildable files
				if xcode.getbuildcategory(node) then
					node.buildid = xcode.newid(prj, node, "build")
				end
    ...
    --printf("Pluging in the Product node and generating new IDs...")
		node = tree.insert(tr.products, prj.xcode.projectnode)
		node.kind = "product"
		node.path = node.cfg.buildtarget.fullpath
		node.cfgsection = xcode.newid(prj, node, "cfg")
		node.resstageid = xcode.newid(prj, node, "rez")
		node.sourcesid  = xcode.newid(prj, node, "src")
		node.fxstageid  = xcode.newid(prj, node, "fxs")
		return tr
	end
...
--
    ...
    testGeneratedID = nil -- Clear the generated ID test table for every new project...
