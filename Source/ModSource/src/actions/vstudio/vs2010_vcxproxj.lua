...
local vs10_helpers = premake.vstudio.vs10_helpers
	...
	function vs10_helpers.sort_input_files(prj,files,sorted_container)
		local types = 
		{	
			h	= "ClInclude",
			hpp	= "ClInclude",
    ...
    --printf("\t\tconfig Kind: %s", config.kind)
    
    if config == nil then
      error("Error while retrieving output type. REASON: Invalid config.")
    end
    
    if t[config.kind] == nil or t[config.kind] == "" then
      error("Error while retrieving output type. REASON: Undefined output type.")
    end
   ...
   local function buffersecuritycheck(cfg)
    ...
    if cfg.flags.Symbols then
      if cfg.kind == "StaticLib" then
        ...
        -- It defaults to intermediates folder
        --_p(4,'<ProgramDataBaseFileName>%s\\%s.pdb</ProgramDataBaseFileName>', "$(IntDir)", "vc100")
      end
    end
    ...
    local targetMachine = iif(cfg.platform == "x64", "MachineX64", "MachineX86")
    ...
    if not cfg.flags.VSNoSubSystem then
      ...
      -- It defaults to $(TargetDir)$(TargetName)
  ...
  --   </ItemGroup>
	...
	local function vcxproj_files(prj)
      ...
      CustomBuild = {}
		...
		vs10_helpers.sort_input_files(prj,cfg.files,sorted)
